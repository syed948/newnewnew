...
// Returns the URL path segments that come after `marker` (e.g. 'auth', 'calls', 'sms', 'users').
// For /api/auth/login this returns ['login']; for /api/admin/users/5/reset-password with
// marker 'users' it returns ['5', 'reset-password'].
//
// Vercel's automatic req.query.action population for catch-all files like [...action].js is
// a Next.js-specific convenience, not something plain (non-framework) Vercel Serverless
// Functions get for free - so this parses req.url directly instead, which works regardless
// of framework. req.query.action is checked first as a bonus in case a given runtime does
// populate it, but the URL parse below is the reliable path.
function getActionSegments(req, marker) {
  if (Array.isArray(req.query?.action) && req.query.action.length) return req.query.action;
...
module.exports = { json, ok, fail, withApi, withAuth, withAdmin, rateLimit, clientIp, getActionSegments };
